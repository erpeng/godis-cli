...
func (client *Client) SubSendCommand(c net.Conn, cmd string, args ...string) {
	// grab a connection from the pool
	var b []byte
...
func (client *Client) sendCommands(cmdArgs <-chan []string, data chan<- interface{}) (err error) {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			// }
			// if args[0] == "publish" {
			// 	client.subSendCommand(c, "PUBLISH", args[1], args[2])
			// }
			// if args[0] == "unsubscribe" || args[0] == "subscribe" {
...
func (client *Client) Ping(args ...string) (string, error) {
	c, err := client.OpenConnection()
	if err != nil {
		return "", fmt.Errorf("Connection faile:%v", err)
	}
	if len(args) == 1 {
		err = writeRequest(c, "PING", args...)
	} else {
		err = writeRequest(c, "PING")
	}
	pong, err := ReadResponse(bufio.NewReader(c))
	switch pong.(type) {
	case string:
		return pong.(string), err
	case []uint8:
		return string(pong.([]uint8)), err
	}
	return "", err
}

func ping(c net.Conn, args ...string) (string, error) {

	if len(args) == 1 {
		writeRequest(c, "PING", args...)
	} else {
		writeRequest(c, "PING")
	}
	// pong, err := readResponse(bufio.NewReader(c))
